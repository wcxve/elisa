...
CURATED_XSPEC_CASES = [
    ...
    },
    {
        ...
        'erange': (0.35, 2.0),
        'name': None,
        'xspec_grouping_supported': False,
    },
    {
        'id': 'NICER/XTI',
...
def _xspec_energy_mask(spectrum, erange: tuple[float, float]) -> np.ndarray:
    energies = np.asarray(spectrum.energies, dtype=np.float64)
    emin = float(erange[0])
    emax = float(erange[1])
    bin_emin = np.minimum(energies[:, 0], energies[:, 1])
    bin_emax = np.maximum(energies[:, 0], energies[:, 1])
    return (bin_emin >= emin) & (bin_emax <= emax)
...
def _xspec_background_counts(spectrum, back_ratio: np.ndarray) -> np.ndarray:
    size = len(back_ratio)
    return (
        _xspec_array(spectrum.background.values, size)
...
def xspec_runtime():
    ...
    os.makedirs(os.path.expanduser('~/.xspec/cache'), exist_ok=True)
...
def test_grouping_matches_xspec(
    ...
    if not case.get('xspec_grouping_supported', True):
        pytest.skip(
            'pyxspec does not expose regrouped spectrum values consistently '
            'for this product'
        )
    mask = _xspec_energy_mask(spectrum, case['erange'])
    xspec_counts = _xspec_source_counts(spectrum)[mask]
    xspec_area_scale = _xspec_array(spectrum.areaScale, len(mask))[mask]
    xspec_back_scale = _xspec_array(spectrum.backScale, len(mask))[mask]
    ...
    xspec_ratio_full = _xspec_back_ratio(spectrum)
    xspec_back_counts_full = _xspec_background_counts(
        spectrum, xspec_ratio_full
    )
    xspec_ratio = xspec_ratio_full[mask]
    xspec_back_counts = xspec_back_counts_full[mask]
    np.testing.assert_allclose(
        xspec_back_counts,
        data.back_counts,
        rtol=rtol,
