...
class Data:
    ...
    def __init__(
        ...
        ancrfile=None,
        name=None,
        is_spec_poisson=None,
        is_back_poisson=None,
        ignore_bad=True,
        keep_channel_info=False,
        group=None,
        scale=None,
    ):
    ...
    def _extract_back(self, backfile, is_back_poisson):
        ...
        if backfile.lower() in ["none", ""]:
            warnings.warn(
                f"assumes {self.name} has no background", stacklevel=2
            )
            self.has_back = False
            return None
    ...
    def _set_name(self, name):
        ...
        if name:
            ...
            _ := self._spec_header["INSTRUME"]
        ) and _.lower() not in excluded:
            self.name = self._spec_header["INSTRUME"]
        elif (
            _ := self._spec_header["TELESCOP"]
        ) and _.lower() not in excluded:
    ...
    def _filter_channel(
        ...
        if group_type != None:
            if group_type not in ["bmin", "bpos"]:
                ...
                if group_type == "bmin" and not (
                    self.has_back and self.back_poisson
                ):
                    raise ValueError(
            ...
            if group_type == "bmin":
                ...
                for mask in chmask:
                    ...
                    grp_error = np.sqrt(
                        np.add.reduceat(error_i * error_i, idx)
                    )
        ...
        resp_matrix = np.add.reduceat(
            self._resp_matrix * factor, self._grouping, axis=1
        )
        ...
        if len(self._grouping) == len(self._spec_counts):  # case of no group
            ...
            # used in fitting.
            any_good_in_group = (
                np.add.reduceat(good_quality, self._grouping) != 0
            )
            groups_edge_indices = np.append(
                self._grouping, len(self._spec_counts)
            )
            ...
            for i in range(len(self._grouping)):
                if not any_good_in_group[i]:
                    continue
                slice_i = slice(
                    groups_edge_indices[i], groups_edge_indices[i + 1]
                )
        ...
        if keep_channel_info:
            groups_channel = np.array(
                [self.name + "_Ch" + "+".join(c) for c in groups_channel]
            )
        else:
            groups_channel = np.array(
                [
