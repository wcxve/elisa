...
def pgstat_background(s, n, b_est, sigma, a):
    ...
    b = pt.switch(
        ...
        pt.switch(pt.gt(n, 0.0), (c + d) / (2 * a), e),
        0.0,
    )
    # b = pt.switch(
    #     pt.gt(n, 0.0),
...
def wstat_background(s, n_on, n_off, a):
    ...
    b = pt.switch(
        ...
        pt.switch(
            pt.eq(n_off, 0),
            pt.switch(
                pt.le(s, a / (a + 1) * n_on), n_on / (1 + a) - s / a, 0.0
            ),
            (c + d) / (2 * a * (a + 1)),
        ),
    )
...
def chi(data, model, context):
    ...
    with context:
        ...
        if data.has_back:
            ...
            error = np.sqrt(
                error * error + ratio * ratio * back_error * back_error
            )

        counts = pm.MutableData(f"{name}_spec_counts", counts)
        error = pm.ConstantData(f"{name}_spec_error", error)
        spec_exposure = pm.ConstantData(
            f"{name}_spec_exposure", data.spec_exposure
        )
...
def cstat(data, model, context):
    ...
    with context:
        ...
        pm.CustomDist(
            ...
            mu_on,
            logp=poisson_logp,
            random=poisson_random,
            observed=spec_counts,
            dims=chdim,
        )
...
def pstat(data, model, context):
    if not data.spec_poisson:
        raise ValueError("Poisson data is required for using P-statistics")
    if not data.has_back:
        raise ValueError("Background is required for using P-statistics")
    ...
    with context:
        ...
        back_exposure = pm.ConstantData(
            f"{name}_back_exposure", data.back_exposure
        )
        ph_ebins = pm.ConstantData(f"{name}_ph_ebins", data.ph_ebins)
        resp_matrix = pm.ConstantData(f"{name}_resp_matrix", data.resp_matrix)
...
def pgstat(data, model, context):
    ...
    if not data.spec_poisson:
        raise ValueError("Poisson data is required for using PG-statistics")
    if not data.has_back:
        raise ValueError("Background is required for using PG-statistics")
...
def wstat(data, model, context):
    ...
    if not data.spec_poisson:
        raise ValueError("Poisson data is required for using W-statistics")
