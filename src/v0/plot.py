...
def plot_ppc(
    ...
    for xi, yi, yhi, yri, color in zip(x, y, y_hat, y_rep, colors):
        ...
        data_resd = az.hdi(
            np.expand_dims(q_data - yhi_cumsum, axis=0), hdi_prob=cl
        ).T
        rep_resd = az.hdi(
            np.expand_dims(q_rep - yhi_cumsum, axis=0), hdi_prob=cl
        ).T
        ...
        for j in range(len(idx) - 1):
            ...
            axes[0].step(
                ebins,
                np.append(q_data[slice_j], q_data[slice_j][-1]),
                lw=0.6,
                where="post",
                color=color,
            )
    ...
    if pars_dist is not None:
        ...
        for cl in [0.683, 0.95]:
            ...
            for i, j in zip(ci_lower, ci_upper):
                ax.fill_between(
                    *i, j[1], lw=0, step="post", color=color, alpha=0.15
                )
...
def _plot_ufspec(
    ...
    ax.step(ebins, np.append(NE, NE[-1]), where="post", c=color, lw=1.3)
    ...
    if comps:
        ...
        if len(NE_comps) > 1:
            for i in NE_comps:
                ax.step(
                    ebins, np.append(i, i[-1]), where="post", c=color, ls=":"
                )
    ...
    if pars_dist is not None:
        ...
        for cl in [0.683, 0.95]:
            ...
            ax.fill_between(
                ebins, *ci, lw=0, step="post", color=color, alpha=0.25
            )
...
def _plot_icounts(
    ...
    if data_sim is not None:
        ...
        for cl in [0.683, 0.95]:
            ...
            for i, j in zip(ci_lower, ci_upper):
                ax.fill_between(
                    *i, j[1], lw=0, step="post", color=color, alpha=0.1
                )

            if cl == 0.95:
                mask = (icounts < ci[0]) | (icounts > ci[1])
                ax.scatter(
                    emid[mask], icounts[mask], marker="x", c="r", zorder=10
                )
    ...
    if data_sim is not None and pars_fit is not None:
        ...
        for cl in [0.683, 0.95]:
            ...
            if cl == 0.95:
                mask = (idiff < ci[0]) | (idiff > ci[1])
                ax.scatter(
                    emid[mask], idiff[mask], marker="x", c="r", zorder=10
                )
...
def _plot_delchi(
    ...
    if data_sim is not None and pars_fit is not None:
        ...
        for cl in [0.683, 0.95]:
            ci = _quantile(
                (sim_CE - model_CE) / sim_CE.std(axis=0, ddof=1),
                cl,
                hdi,
                axis=0,
            )
            ...
            if cl == 0.95:
                ...
                ax.scatter(
                    emid[mask], delchi[mask], marker="x", c="r", zorder=10
                )
    ...
    if data_sim is not None and pars_fit is not None:
        ...
        for cl in [0.683, 0.95]:
            ...
            if cl == 0.95:
                ...
                ax.scatter(
                    emid[mask], ratio[mask], marker="x", c="r", zorder=10
                )
    ...
    if data_sim is not None and pars_fit is not None:
        ...
        for cl in [0.683, 0.95]:
            ...
            if cl == 0.95:
                ...
                ax.scatter(
                    emid[mask], diff[mask], marker="x", c="r", zorder=10
                )
    ...
    if deviance_dist is not None:
        for cl in [0.683, 0.95]:
            ...
            for i in _split_step_plot(data.ch_emin, data.ch_emax, upper):
                ui = np.sqrt(i[1])
                ax.fill_between(
                    i[0], -ui, ui, lw=0, step="post", color=color, alpha=0.15
                )
