...
src_models = {
    ...
    "PL+BB": EnergyFlux(1.5, 250) * Powerlaw(norm=1)
    + EnergyFlux(1.5, 250) * BlackBodyRad(norm=1),
    "BB+BB": EnergyFlux(1.5, 250) * BlackBodyRad(norm=1)
    + EnergyFlux(1.5, 250) * BlackBodyRad(norm=1),
}
...
for m in list(src_models.keys()):
        ...
        for i in range(1, n + 1):
            ...
            if data_list:
                try:
                    ...
                    p_res = np.append(
                        infer._ppc_result["p_value"]["total"],
                    ...
                    if not os.path.exists(f"{result_path}/{obsid}_{i:02d}"):
                        os.mkdir(f"{result_path}/{obsid}_{i:02d}")
                    infer.plot_data(
                    ...
                    with open(
                        ...
                        mle_res = {
                            k: v
                            for k, v in infer._mle_result.items()
                            if k != "minuit"
                        }
                        pickle.dump(
                            [mle_res, infer._idata, infer._ppc_result], f
                        )
    ...
    print("\n\n\n")
    pars_name = [
        j for i in infer._rv["name"] for j in [i, "se", "lower", "upper"]
    ]
    res_i.insert(
        0,
        [
