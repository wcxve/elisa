...
class XspecNumericGradOp(NumericGradOp):
    ...
    def __init__(self, grad_method="f", eps=1e-7, **par_kwargs):
        ...
        self.itypes = [pt.TensorType("floatX", shape=()) for _ in pars]
        self.itypes.append(pt.TensorType("floatX", shape=(None,)))  # ebins

        if self.optype == "con":
            self.itypes.append(pt.TensorType("floatX", shape=(None,)))  # flux
