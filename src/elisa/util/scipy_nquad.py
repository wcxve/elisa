...
class NQuadTransform:
    def __init__(self, fun):
        self._c_sig = nb.types.double(nb.types.intc, 
    ...
    def _nquad(cfun, opts=None):
        ...
        def _nquad_scipy(ranges,args):
            ...
            def _pcb(ranges, args):
                ...
                result, abserr = integrate.nquad(fun_sci_call, ranges, args, opts, full_output=False)
                return jnp.array([result, abserr])
            result_shape_dtype = jax.ShapeDtypeStruct(shape=(2,), dtype=jnp.float64)
            return jax.pure_callback(_pcb, result_shape_dtype, 
                                            ranges,args, 
                                            vectorized=False)
        return _nquad_scipy
...
if __name__=='__main__':
    ...
    ranges = jnp.array([[0.0,1.0],[0.0,1.0]], dtype=jnp.float64)
    ...
    '''example 2'''
    from elisa.models.model import AnaIntAdditive,NumIntAdditive,ParamConfig
    ...
    class BlackbodyRad_test(AnaIntAdditive):
        ...
        def integral(egrid: JAXArray, params: NameValMapping) -> JAXArray:
            ...
            ranges = jnp.array([egrid[:-1],egrid[1:]], dtype=jnp.float64).T
            ranges = jnp.reshape(ranges, (len(ranges),1,2))
            args = jnp.array([kT,K], dtype=jnp.float64)
