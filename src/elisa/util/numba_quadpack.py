...
class QuadpackTransform:
    ...
    def _dqng(cfun, vectorized=False):
        from NumbaQuadpack import dqng
        @jax.jit
        def _dqng_call(a, b, data = jnp.asarray([0.0], jnp.float64), 
                            epsabs = 1.49e-08, epsrel = 1.49e-08):
            a, b = jnp.float64(a), jnp.float64(b)
            epsabs, epsrel = jnp.float64(epsabs), jnp.float64(epsrel)
            data = jnp.asarray(data)
    ...
    def _dqag(cfun, vectorized=False):
        from NumbaQuadpack import dqag
        @jax.jit
        def _dqag_call(a, b, data = jnp.asarray([0.0], jnp.float64), 
                            epsabs = 1.49e-08, epsrel = 1.49e-08, irule=1):
            a, b = jnp.float64(a), jnp.float64(b)
            epsabs, epsrel = jnp.float64(epsabs), jnp.float64(epsrel)
            ...
            def _pcb(a, b, data, epsabs, epsrel, irule):
                ...
                data = np.asarray(data)
                sol, abserr, success = dqag(cfun, a, b, data, 
                                            epsabs, epsrel, irule)
                return np.asarray([sol, abserr, success], dtype=np.float64)
            ...
            return jax.pure_callback(
                _pcb, result_shape_dtype, a, b, data, 
                                    epsabs, epsrel, irule, 
                                    vectorized=vectorized
            )
        return _dqag_call
